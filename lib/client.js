...
const { EVENT_RDS_REQUEST, EVENT_RDS_RESPONSE } = require('./constant');
...
proto.getConnection = function() {
  ...
  function onConnection(conn) {
    ...
    rdsConn.on(EVENT_RDS_REQUEST, that.emit.bind(that, EVENT_RDS_REQUEST));
    rdsConn.on(EVENT_RDS_RESPONSE, that.emit.bind(that, EVENT_RDS_RESPONSE));
    return rdsConn;
  }
...
proto.beginTransaction = function* () {
  ...
  const rdsTrans = new RDSTransaction(conn);
  rdsTrans.on(EVENT_RDS_REQUEST, this.emit.bind(this, EVENT_RDS_REQUEST));
  rdsTrans.on(EVENT_RDS_RESPONSE, this.emit.bind(this, EVENT_RDS_RESPONSE));
  return rdsTrans;
};
...
proto.end = function(callback) {
  ...
  this.removeAllListeners(EVENT_RDS_REQUEST);
  this.removeAllListeners(EVENT_RDS_RESPONSE);
  // callback style
  if (callback) {
