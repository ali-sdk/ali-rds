...
function RDSClient(options) {
  ...
  this.pool.getConnection = async function() {
    ...
    function onQuery(conn) {
      if (!_hook || !_hook.callback || !_hook.callback.onQuery) return;
      if (conn.__hook_onQuery) return;
      conn.__hook_onQuery = true;
      const _query = conn.query;
      conn.query = async function(sql, values) {
        const prevTime = Number(new Date());
        const result = await _query.call(conn, sql, values);
        const ms = Number(new Date()) - prevTime;
        _hook.callback.onQuery(_hook, ms, 'todo');
        return result;
      };
    }
    //
