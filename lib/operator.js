...
proto.query = function* (sql, values) {
  ...
  try {
    this.emit(EVENT_RDS_REQUEST, { sql, values });
    const rows = yield this._query(sql);
    execDuration = Date.now() - queryStart;
    this.emit(EVENT_RDS_RESPONSE, { sql, values, rows, execDuration });
    debug('query get %d rows', rows.length);
    return rows;
  } catch (err) {
    execDuration = Date.now() - queryStart;
    this.emit(EVENT_RDS_RESPONSE, { sql, values, error: err, execDuration });
    err.stack = err.stack + '\n    sql: ' + sql;
